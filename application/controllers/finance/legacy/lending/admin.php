...
$fields_header = array('name','username','usergroup','mpassword','mpassword_confirm','enable','date_expire','branch_id','branch_id2','branch_id3');
...
elseif ($p1 == 'Save User' && $aadmin['admin_id'] == '')
{
	...
	$q = "insert into admin (name, username, usergroup, mpassword, date_expire, branch_id, branch_id2, branch_id3, enable)
				values ('".$aadmin['name']."','".$aadmin['username']."','".$aadmin['usergroup']."',
						'$mpasswd', '".$aadmin['date_expire']."', '".$aadmin['branch_id']."', '".$aadmin['branch_id2']."', '".$aadmin['branch_id3']."',
						'".$aadmin['enable']."')";
	$qr = @pg_query($q) or message(pg_errormessage().$q);
...
elseif ($p1 == 'Save User' && $aadmin['admin_id'] != '')
{
	...
	$q = "update admin set name='".$aadmin['name']."',
							...
							branch_id2 = '".$aadmin['branch_id2']."',
							branch_id3 = '".$aadmin['branch_id3']."',
...
elseif ($p1 == 'Ok')
{
		...
		if ($iadmin['adminrights_id'] != '') 
		{
			...
			$qr = @pg_query($sql) or message(pg_errormessage().$q);
		}
		else
		{
			$sql = "insert into adminrights (admin_id, module_id, madd, medit, mdelete, mview)
						values ( '".$aadmin['admin_id']."', '".$iadmin['module_id']."',
						 '$_add', '$_edit', '$_delete', '$_view')";
			$qr = @pg_query($sql) or message(pg_errormessage());
		...
		if ($qr)
		{
			...
			@audit($module, $sql, $ADMIN['admin_id'], $msg, $iadmin['adminrights_id'] );
  ...
  <table width="70%" border="0" align="center" cellpadding="1" cellspacing="1" bgcolor="#EFEFEF">
    ...
    <tr bgcolor="#FFFFFF"> 
      ...
      <td> 
        ...
        <? //= lookUpAssoc('usergroup',array('Loan Staff'=>'L','Payment/Collection'=>'C','Supervisor'=>'S','Administrator'=>'A'),$aadmin['usergroup']);?>      </td>
    </tr>
    <tr bgcolor="#FFFFFF">
      <td><font size="2" face="Verdana, Arial, Helvetica, sans-serif">Branch #1 </font></td>
    ...
    <tr bgcolor="#FFFFFF">
      <td><font size="2" face="Verdana, Arial, Helvetica, sans-serif">Branch #2</font></td>
      <td><select name="branch_id2" id="branch_id2">
        <option value="0">Select Branch</option>
        <?
			$q = "select * from branch where enable='Y' and branch_id != '$branch_id2' order by branch";
			$qr = @pg_query($q);
			while ($r = @pg_fetch_object($qr))
			{
				if ($aadmin['branch_id'] == $r->branch_id)
    ...
    <tr bgcolor="#FFFFFF"> 
      <td><font size="2" face="Verdana, Arial, Helvetica, sans-serif">Branch #3</font></td>
      <td><select name="branch_id3" id="branch_id3">
        <option value="0">Select Branch</option>
        <?
			if ($branch_id2 != 0)
			{
				$q = "select * from branch where enable='Y' and branch_id != '$branch_id' and branch_id != '$branch_id2' order by branch";
			} else		$q = "select * from branch where enable='Y' and branch_id != '$branch_id' order by branch";
			$qr = @pg_query($q);
			while ($r = @pg_fetch_object($qr))
			{
				if ($aadmin['branch_id'] == $r->branch_id)
    ...
    <tr bgcolor="#FFFFFF"> 
      ...
      <td> 
        <?= lookUpAssoc('enable',array('Yes'=>'Y','No'=>'N'), $aadmin['enable']);?>      </td>
    </tr>
    <tr bgcolor="#FFFFFF"> 
