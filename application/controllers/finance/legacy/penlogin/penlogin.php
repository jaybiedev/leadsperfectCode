...
if ($aid != '')
{
	$q = "select * from account where account_id ='$aid' and enable";
...
{
		$q = "select * from account where smartno ='$pin' and enable";
		$qr = @pg_query($q) or message(pg_errormessage());
		if (@pg_num_rows($qr) == '0')
		{
			echo "<script>alert('Account NOT found...')</script>";
		...
		if ($r['pensionpin'] == $pwd)
		{
			...
			if ($flg==1)
			{
				...
				if (@pg_num_rows($qrs) == 0)
				{
					$qs = "insert into schedule (branch_id,account_id,smartno,status,date,timeref,schednum,active,admin_id)
										values ('".$ADMIN['branch_id']."',
												'".$aPenlogin['account_id']."',
												'".$aPenlogin['smartno']."','Queued',
