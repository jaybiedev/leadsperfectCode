...
if ($p1=='Set  Selected Active Globally')
{
	...
	$q = "update payroll_period set active='0', audit='$audit' where active>'0'" ;

	@pg_query($q)or message(pg_errormessage());
	
	$q = "update payroll_period set active='1', audit='$audit' where payroll_period_id='$active' and enable='Y'";
  ...
  <table width="60%" border="0" align="center" cellpadding="1" cellspacing="1">
    ...
    <tr bgcolor="#FFFFFF"> 
      <td align="center"><font size="2" face="Verdana, Arial, Helvetica, sans-serif"> 
        <input type="radio" name="aactive[]" value="<?=$r->payroll_period_id;?>" <?=($r->active=='1' ? 'checked' :'');?>>
        </font></td>
      <td align="center"><font size="2" face="Verdana, Arial, Helvetica, sans-serif">
        <input type="radio" name="alocal_active[]" value="<?=$r->payroll_period_id;?>" <?=($local_active==$r->payroll_period_id ? 'checked' :'');?>>
        </font></td>
