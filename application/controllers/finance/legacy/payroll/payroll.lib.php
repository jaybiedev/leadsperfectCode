...
function sumAnnual($date_from, $date_to,$array_paymast_id)
{
	$asum = null;
	$asum = array();
	...
	while ($r = @pg_fetch_object($qr))
	{
		...
		{	
			...
			while ($rr = @pg_fetch_object($qqr))
			{
				...
				if ($rr->type == 'I')
				{
					$icode = 'I'.$ri->income_code;
					$dummy[$icode] += $amount;					
				} else
				{
					$dcode = 'D'.$ri->deduction_code;
					$dummy[$dcode] += $amount;
				}
/*if ($rr->payroll_header_id==37204)
{
	print_r($dummy);
	echo "<br>";
}*/					
...
function computeDeduction($aPM)
{
	...
	$qpm = "select * from paymast where paymast_id = '".$aPM['paymast_id']."'";
	$qpmr = @pg_query($qpm) or message(pg_errormessage());
	$rpm = @pg_fetch_object($qpmr);
	...
	if ($aPM['pagibigw'] == '1')
	{
		...
		if ($rpm->fixed_pagibig > 0)
		{
			$aPM['total_pagibig'] = $rpm->fixed_pagibig;
		}
		else
		{
	
			$aPM['total_pagibig']=100;
		}
	}
	else
	{
