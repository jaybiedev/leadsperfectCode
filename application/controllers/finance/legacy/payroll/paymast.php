...
<?
if (!chkRights2("paymast","mview",$ADMIN['admin_id']) and !chkRights2("hrempmast","mview",$ADMIN['admin_id']))
{
...
if (!in_array($p1, array('','Edit','Print')))
{
	$fields = array(  'idnum', 'etype', 'branch_id', 'department_id', 'section_id', 'efirst', 'pay_category','rank',
			...
			'fixed_sss','fixed_phic','fixed_wtax','fixed_pagibig','fixed_ssse','fixed_phice','fixed_wtaxe','fixed_pagibige',
			'laundry','uniform','rice','medical','medepend','meal','management','tenure','dcola','transpo','dayear',
			'date_resign','tempbranch_id','biomatrix','course');
	...
	for($c=0;$c<count($fields);$c++)
	{
		...
		if ( $paymast[$item] == '' && in_array($item, array('etype','department_id','section_id','level_id','civil_status',
								'emp_status','ratem','hourly','adwr','cola','branch_id','tenureallowance',
								'fixed_sss','fixed_phic','fixed_wtax','fixed_pagibig','fixed_ssse','fixed_phice','fixed_wtaxe',
								'fixed_pagibige','laundry','uniform','rice','medical','medepend','meal','management','tenure',
								'dcola','transpo','dayear')))
		{
			$paymast[$item] = 0;
		}
	}
	$pixname = $paymast['idnum'];
	$date1=$paymast['date_birth'];
	$date2=date("Y-m-d");
	if ($date1 != '--')
	{
		$qq = query("SELECT date_part ('year', f) * 12 + date_part ('month', f) as age
							FROM age ('$date2'::DATE, '$date1'::DATE) f ");
		$rr = @pg_fetch_object($qq);
		$age = intval($rr->age/12);
	}	
}
...
elseif ($p1=='Save' && ($paymast['elast'] == '' || $paymast['efirst'] == ''))
{
	message1("No Employee Name Specified...");
}	
elseif ($p1=='Save')
{	
	if ($paymast[date_resign]=='' or $paymast[date_resign]=='--') $paymast[date_resign]='0000-00-00';
	if ($paymast[tempbranch_id]=='') $paymast[tempbranch_id]=0;
	if ($paymast['paymast_id'] == '') 
	{
		if (!chkRights2("paymast","madd",$ADMIN['admin_id']))
		{
			message("You have no permission in this area...");
			exit;
		}
	...
	{
		if (!chkRights2("paymast","medit",$ADMIN['admin_id']) and !chkRights2("hrempmast","medit",$ADMIN['admin_id']))
		{
			message("You have no permission in this area...");
			exit;
		}
...
if (chkRights2("hrview","mview",$ADMIN['admin_id'])) 
{
	$rghts='hidden';
} else
{
...
<form name="f1" id="f1" method="post" action="" style="margin:0"  enctype="multipart/form-data" >
  ...
  <table width="95%" border="0" cellspacing="0" cellpadding="0" align="center">
      <td colspan="2"> <table width="100%" cellpadding="0" cellspacing="1" bgcolor="#EFEFEF">
          <tr bgColor=<?=($paymast['enable'] == 'N'?'#FFCCCC':'');?>> 
    ...
    <tr> 
      <td colspan="2" height="400px" valign="top"> <div class="tabber" style="width:95%; left:20px"> 
          ...
          <div class="tabbertab"> 
            ...
            <p>
			<?
			if (chkRights2("hrview","mview",$ADMIN['admin_id']) and $ADMIN[usergroup]!='A' and $ADMIN[usergroup]!='M') 
			{
				$rights=0;
			} else
			{
			?>	
              <? include_once('paymast.defaults.php');?>
			<?
			}
			?>  
            </p>
