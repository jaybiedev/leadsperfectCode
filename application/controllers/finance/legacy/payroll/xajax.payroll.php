<?
function paytransdefault($form)
{
	$income_type_id = $form['income_type_id'];
	$paymast_id = $form['paymast_id'];
galert($income_type_id);
	if ($income_type_id == '' or $income_type_id== '0')
	{
		return done();
	}
	$q= "select * from income_type where income_type_id = '$income_type_id'";
	$qr = @pg_query($q);
	$r = @pg_fetch_object($qr);
	$income_code = $r->income_code;
	$q= "select * from paymast where paymast_id = '$paymast_id'";
	$qr = @pg_query($q);
	$r = @pg_fetch_object($qr);
	
	if ($income_code == 'A10') 	   gset('income_amount',$r->laundry);
	elseif ($income_code == 'A7')  gset('income_amount',$r->uniform);
	elseif ($income_code == 'A6')  gset('income_amount',$r->rice);
	elseif ($income_code == 'A8')  gset('income_amount',$r->medical);
	elseif ($income_code == 'A9')  gset('income_amount',$r->medepend);
	elseif ($income_code == 'A13') gset('income_amount',$r->meal);
	elseif ($income_code == 'A14') gset('income_amount',$r->management);
	elseif ($income_code == 'B1')  gset('income_amount',$r->tenure);
galert($q.'  '.$income_code);
	return done();
